...
export default async function AdminPage(){
 ...
 return <main className="page"><a className="back" href="/">← HOME</a><p className="eyebrow">IRONFORGE / ADMIN</p><h1>CONTROL <i>CENTER.</i></h1><p className="pageLead">Production management dashboard for {profile.full_name||"administrator"}.</p><section className="pageGrid">{[["MEMBERS",members],["ACTIVE MEMBERSHIPS",activeMemberships],["CONFIRMED BOOKINGS",bookings],["NEW ENQUIRIES",messages],["TRAINING REQUESTS",trainingRequests],["SUCCESSFUL PAYMENTS",payments]].map(([label,count])=><div className="card" key={String(label)}><span className="num">{label}</span><h3>{count??0}</h3><p>Live database count</p></div>)}</section></main>
}
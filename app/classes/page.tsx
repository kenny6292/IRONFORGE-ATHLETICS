...
export default async function Classes(){
 ...
 return <main className="page"><a className="back" href="/">← HOME</a><p className="eyebrow">IRONFORGE / SCHEDULE</p><h1>CLASS <i>LINEUP.</i></h1><p className="pageLead">Live classes from the IronForge database. Sign in to reserve your place.</p><div className="classList">{classes?.map(c=>{const trainer=Array.isArray(c.trainers)?c.trainers[0]:c.trainers; return <div className="classRow" key={c.id}><span className="classNum">{dayNames[c.day_of_week]||"DAY"}</span><div><h3>{c.name}</h3><p>{c.start_time} · {c.duration_minutes} min · {trainer?.name||"IRONFORGE COACH"}</p><small>{c.description||"Performance-focused training session."} · Capacity {c.capacity}</small></div><a className="btn primary" href="/login?next=/classes">BOOK</a></div>})}</div></main>
}
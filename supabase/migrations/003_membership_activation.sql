-- Membership activation is performed only after a verified payment.
...
begin
  ...
  where reference = p_reference and status = 'successful'
  for update;

  if v_payment.id is null then
    raise exception 'Verified payment not found';
  end if;

  if v_payment.user_id is null then
    raise exception 'Payment has no member';
  end if;

  v_plan_id := nullif(v_payment.metadata->>'plan_id','')::uuid;
  if v_plan_id is null then
    raise exception 'Payment has no membership plan';
  end if;
...
revoke all on function public.activate_membership_for_payment(text) from public;
grant execute on function public.activate_membership_for_payment(text) to authenticated;